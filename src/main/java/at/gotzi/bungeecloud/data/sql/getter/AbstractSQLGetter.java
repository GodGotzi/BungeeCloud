...
public abstract class AbstractSQLGetter<T> implements ISQLGetter {
    ...
    public AbstractSQLGetter(String table, T obj, String val, String key, String nativeData, BungeeCloud bungeeCloud) {
        this.table = table;
        this.obj = obj;
        ...
        try {
            this.bungeeCloud.getSqlHandler().connect();
        } catch (SQLException e) {
            this.bungeeCloud.getGotziLogger().warning("Sql cannot connect", this.getClass());
    ...
    public void create() {
        ...
        try {
            if (!exists()) {
                PreparedStatement ps1 = this.bungeeCloud.getSqlHandler().getConnection().prepareStatement(perform.toString());
                ps1.executeUpdate();
            }
        } catch (SQLException e) {
    ...
    public void update(int o, String from) {
        ...
        try {
            PreparedStatement ps = this.bungeeCloud.getSqlHandler().getConnection().prepareStatement(perform.toString());
            ps.setInt(1, o);
    ...
    public boolean exists() {
        ...
        try {
            PreparedStatement ps = this.bungeeCloud.getSqlHandler().getConnection().prepareStatement(stringBuilder.toString());
            ps.setString(1, this.obj.toString());
    ...
    public Value<?> getValue(SQLHandler.DataType dataType) {
        ...
        try {
            PreparedStatement ps =this.bungeeCloud.getSqlHandler().getConnection().prepareCall(stringBuilder.toString());
